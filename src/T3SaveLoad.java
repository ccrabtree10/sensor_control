...
public class T3SaveLoad 
{
	...
	public static void saveLoad() throws FileNotFoundException
	{
		...
		kryo.register(JComboBox.class, new JavaSerializer(), 7);
		
		
		
		// !!! MAKE OBJECT TO SAVE !!!
		//MidiOutModule objectToWrite = new MidiOutModule();
		//GraphModule objectToWrite = new GraphModule(new MidiOutModule("test"));
		ArrayList objectToWrite = new ArrayList();
		GraphModule graphModule = new GraphModule(new StMcModule());
		objectToWrite.add(graphModule);
		...
		GraphModule gmod = (GraphModule) array.get(0);
		gmod.initialize();
